...
* `cobbler_system.interface = { ... }` map removed — split each interface into
  its own `cobbler_network_interface` resource. Network interfaces are a flat,
  top-level Cobbler collection, so `name` must be globally unique across all
  systems (e.g. `<ifname>-<systemname>`); Cobbler's name validation rejects `@`.
...
* `boot_files` and `fetchable_files` attributes removed from `cobbler_distro`,
  `cobbler_image`, `cobbler_menu`, `cobbler_profile`, `cobbler_system` (both
  resources and data sources) — the fields were dropped upstream in Cobbler
  4.0.0 with no replacement.
* `template_files` on `cobbler_distro`, `cobbler_image`, `cobbler_menu`,
  `cobbler_profile`, `cobbler_system` is no longer inheritable — it is now a
  plain string map (`{ key = "value" }`) instead of the `{ value = {...},
  inherited = bool }` object form.
* `cobbler_profile.name_servers_search` is no longer inheritable — same
  plain-list-instead-of-object change as above (`cobbler_system.name_servers_search`
  was already a plain list and is unaffected).
* `cobbler_profile.distro`, `cobbler_profile.parent`, `cobbler_system.profile`,
  `cobbler_system.image` now take the referenced resource's Cobbler UID
  (e.g. `cobbler_distro.foo.uid`) instead of its name. Referencing by name
  meant renaming the target object produced a permanent plan diff (and could
  force recreation) since Cobbler's own API is UID-only for these fields;
  cobblerclient no longer papers over that with a name↔UID translation layer.
  New computed `uid` attributes were added to `cobbler_distro`, `cobbler_profile`,
  `cobbler_image`, and `cobbler_menu` to support this (`cobbler_system` already
  had one). `cobbler_image.menu` is similarly now UID-based.
